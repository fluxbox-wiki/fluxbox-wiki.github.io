...
[http://voins.program.ru/wdm/ WDM] is a small, light-weight replacement for XDM. If you're looking for  nice simple replacement to XDM, i would highly recommend it.  


Its fairly easy to add fluxbox (or any other window manager) to its list when logging in.

__TOC__
I assume you've got WDM installed and working properly. If not, go do that and then come back.


Firstly, you'll need to edit two of the config files for wdm. The first being '''Xclients''' and the second being '''wdm-config'''. On my distribution ([http://www.gentoo.org Gentoo]) these are located in '''/etc/X11/wdm'''. 
