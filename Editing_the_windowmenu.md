...
Windowmenu is the menu you get when you right click on a window's titlebar, press the menu-button on the titlebar, use the :WindowMenu keybind or even from the workspaces menu.
By default it has options to maximize, minimize, shade etc windows, send them to different layers or workspaces and to rembember window's options.
...
==Editing the menu==
Windowmenu is like the [[howto edit menu|rootmenu]], except it has some special items that normal menu doesn't.


The menu should start with '''[begin]''' and end with '''[end]'''.

You can use the [[Editing_the_menu#What_really_is_possible.3F|normal menuitems]] or special ones from the list below. Note that (label) is optional, without it a default one will be used:
 [shade] (label)
