...
== Introduction ==
Fluxbox' tabs aren't in any way a new idea. The implementation is much like that of the PWM Window Manager. <br>
...
== How? ==
To move a window into another, to form a group, you use ctrl-left_click (see "OnTitlebar Control Mouse1 :StartTabbing" in <code>~/.fluxbox/keys</code>) on a window's tab (or titlebar)
and drag it to the desired window. In the same way you can ungroup a set of windows, by dragging the tab away to a free space on the desktop, or in another window or group. <br>
...
In <code>~/.fluxbox/init</code> change or add this variable:
...
== Autogrouping ==
Via [http://fluxbox-wiki.org/index.php?title=Editing_the_apps_file#Grouping_apps_via_the_apps_file <code>~/.fluxbox/apps</code>], windows can be grouped according to certain specific criteria.
...
== Style ==
To utilize fluxbox' neat tabs and make them look (even more) pretty you'll need to add some extra entries to your desired style (theme). <br>
