...
There are many ways to install idesk. Users without experience perhaps prefer using binaries packages, depending on the distro they are working with. The other option is to download the sources from [http://sourceforge.net/projects/idesk/ idesk site]
To download binaries [http://www.rpmseek.com rpmseek] or [http://www.rpmsearch.com rpmsearch]
...
Installing from sources is pretty easy too.
We decompress and untar the tar.gz file into any directory. Inside that directory:
...
Now it's time to start tweaking our idesk to make it functional.
With our favourite editor we will create a file called .ideskrc inside our home directory.
...
If we look well we will realize that we can play with the values of the variables. As a hint, Transparency goes from 0 to 255. 0 gives as solid color and 255 will give us transparent icons that will only be seen with mouse over them.
For a more detailed explanation we have to look at readme file locatead in '''/usr/share/doc/idesk'''.

==Creating icons and associated actions==

Now the only thing to do is define icons what is that we want them to do.
Inside home directory we have to create a directory called .idesktop
...
We've just created an icon. If we want it to launch Mozilla Firefox for example we have to do the following.
Inside the file recentely created we have to write:
...
That's it. We are ready to use idesk. 
